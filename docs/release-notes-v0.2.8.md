...
- 修复 v0.2.6 将 automation 参数混入 Firestone 启动命令的问题；不再执行 `-launchapp <AppId> -from-desktop --automation 18765 --enable-automation`。
- Automation runtime 初始化与 Firestone 应用启动已拆成两个阶段：先在本次有效端口执行 `Overwolf.exe --automation <effectivePort> --enable-automation`，再严格执行 `OverwolfLauncher.exe -launchapp <AppId> -from-desktop`。
...
- 启动等待由两种启动器各等待 30 秒收敛为一次 25 秒，不再杀掉已经正常打开的 Firestone 后轮询未经验证的参数组合。
